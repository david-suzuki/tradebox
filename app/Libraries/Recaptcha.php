...
class Recaptcha
{
    ...
    public function __construct()
    {
        ...
        $recaptcha = $this->_db->table('dbt_security')->select('*')->where('keyword', 'capture')->get()->getRow();
        $security_decode = json_decode($recaptcha->data, TRUE);

        $language = $this->_db->table('setting')->select('*')->get()->getRow();
        ...
        $this->_siteKey   = @$security_decode['site_key'];;
        $this->_secretKey = @$security_decode['secret_key'];
        $this->_language  = @$lang;
    ...
    {
        $remoteIp = (!empty($remoteIp)) ? $remoteIp : $this->_db->input->ip_address();
