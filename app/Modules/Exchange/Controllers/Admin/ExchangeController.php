...
class ExchangeController extends BaseController
{
    ...
    public function coin_pair(){
        ...
        $page_number      = (!empty($this->request->getGet('page'))?$this->request->getGet('page'):1);
        $data['coinpair'] = $this->common_model->get_all('dbt_coinpair', array(), 'order','asc',20,($page_number-1)*20);
    ...
    { 
        ...
        $max_order = $this->db->table('dbt_coinpair')->selectMax('order')->get()->getRow();
        if ($max_order) {
    ...
    public function market_streamer()
    {
        ...
        $sql = "SELECT * FROM `dbt_coinhistory`
            INNER JOIN (SELECT `market_symbol`, MAX(`id`) AS maxid
                FROM `dbt_coinhistory` GROUP BY `id`,`market_symbol`) topid
            ON dbt_coinhistory.`market_symbol` = topid.`market_symbol`
                AND dbt_coinhistory.`id` = topid.`maxid`";
    ...
    public function row_order()
    {
        ...
        $order = $this->db->table('dbt_coinpair')->select('order')
                        ->where('id', $id)->limit(1)->get()->getRow();
        if (!$order) {
            return $this->response->setJSON([
                'success'=> false,
            ]);
        }
        $order_new = $this->db->table('dbt_coinpair')
            ->select(['id', 'order'])
            ->where('order '.($mode>0?'>':'<'), $order->order)
            ->orderBy('order', ($mode>0?'asc':'desc'))
            ->limit(1)->get()->getRow();
        if (!$order_new || $order == $order_new->order) {
            return $this->response->setJSON([
                'success'=> false,
            ]);
        }
        $this->db->table('dbt_coinpair')->set('order', $order->order)->where('id', $order_new->id)->update();
        $this->db->table('dbt_coinpair')->set('order', $order_new->order)->where('id', $id)->update();
        return $this->response->setJSON([
            'success'=> true,
        ]);
    }
