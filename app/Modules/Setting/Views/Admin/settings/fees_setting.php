                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-lg-3">
                            ...
                            <select class="form-control" name="level" required >
                               <option value="" hidden>--<?php echo display("select_level") ?>--</option>
                        ...
                        <div class="form-group col-lg-3">
                            <label class="font-weight-600"><?php echo display("select_type") ?> <i class="text-danger">*</i></label>
                            <select class="form-control" name="feetype" required >
                               <option value="" hidden>--<?php echo display("select_type") ?>--</option>
                               <option value="PERCENT"><?php echo "%" ?></option>
                            </select>
                        </div>
                        <div class="form-group col-lg-3">
                            <label class="font-weight-600"><?php echo display("fees") ?><i class="text-danger">*</i></label>
                            <input type="number" class="form-control" name="fees" min="0" onkeypress="return event.charCode != 45" required >
                        </div>
                    </div>
                    <div>
                ...
                <table id="example" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-left"><?php echo esc($value->fees).($value->feetype==='PERCENT'?'%': ' '.$value->feetype);?></td>
...
<script>
    $(document).ready(function() {
        const select_level_element = $("select[name=level]");
        select_level_element.on("change", function() {
            const select_level_val = select_level_element.val();
            if (select_level_val === "WITHDRAW") {
                $("select[name=feetype]").empty();
                $("select[name=feetype]").append('<option value="" hidden>--<?php echo display("select_type") ?>--</option>')
                $("select[name=feetype]").append('<option value="USDE"><?php echo display("by_usde") ?></option>')
                $("select[name=feetype]").append('<option value="USDC"><?php echo display("by_usdc") ?></option>')
                $("select[name=feetype]").append('<option value="USDT"><?php echo display("by_usdt") ?></option>')
            } else {
                $("select[name=feetype]").empty();
                $("select[name=feetype]").append('<option value="" hidden>--<?php echo display("select_type") ?>--</option>')
                $("select[name=feetype]").append('<option value="PERCENT"><?php echo "%" ?></option>')
            }
        })
    })
</script>