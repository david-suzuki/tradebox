...
class SettingController extends BaseController
{
    ...
    public function fees_settig(){
        ...
        $data['coins']     = $this->common_model->findAllGroupBy('dbt_cryptocoin', array('status' => 1), 'symbol', 'rank', 'asc', 100, 0);
        $page_number      = (!empty($this->request->getGet('page'))?$this->request->getGet('page'):1);
        //findAll paramiter = where, limit, offset.
    ...
    public function fees_setting_save()
    {
        ...
        $this->validation->setRule('fees', "Fees",'required|trim|max_length[20]');
        ...
        if ($this->validation->withRequest($this->request)->run()) {
            ...
            if(!empty($check)){
                ...
                $fees = array(
                    ...
                    'feetype'=>$this->request->getPost('feetype', FILTER_SANITIZE_STRING),
    ...
    public function transaction_setup(){
        ...
        $condition      = "proof_type !='erc20' AND proof_type !='bep20' AND status != '2'";
        $data['coins']  = $this->common_model->findAllGroupBy('dbt_cryptocoin', $condition , 'symbol', 'rank', 'asc');
    ...
    public function update_external_api($id = null)
    {
        ...
        $data['apis'] = (object)$userdata = array(
            ...
            'data'   => json_encode(array('api_key'=> $this->request->getPost('api_key',FILTER_SANITIZE_STRING),'create_link'=> $api_data['create_link'])),
            'status' => $this->request->getPost('status',FILTER_SANITIZE_STRING)
