        ...
        if($this->request->getMethod() == 'post'){
            ...
            if ($this->validate($rules, $rules))
            {
                ...
                if (empty($article_id))
                {
                    ...
                    return  redirect()->to(base_url('/backend/cms/faq-list'));
                } else {
                    if ($this->common_model->update('web_article', $userdata, array('article_id' => $article_id))) {
                        $this->session->setFlashdata('message', display('update_successfully'));
                    } else {
                    ...
                    return  redirect()->to(base_url('/backend/cms/edit-faq/'.$article_id));
                }
            } else {
                ...
                if(empty($article_id)){
                    return  redirect()->to(base_url('/backend/cms/add-faq'));
                } else {
                    return  redirect()->to(base_url('/backend/cms/edit-faq/'.$article_id));
                }
            }
    ...
    */
    ...
    public function add_advertisement($id = null)
    {
        ...
        if (!empty($id)) {
            ...
            if ($exist) {
                $this->session->setFlashdata('exception', display('already_exists'));
               return redirect("backend/cms/add-advertisement");
            }
        }
        ...
        if($this->request->getMethod() == 'post'){
            ...
            if ($this->validation->withRequest($this->request)->run()) {
                ...
                $this->session->setFlashdata("exception", $this->validation->listErrors());
                return  redirect()->to(base_url('/backend/cms/advertisement-list'));
            }
