...
helper('common');
use App\Libraries\CoinMarketCap;
class HomeController extends BaseController
{
    ...
    public function exchange_page()
    {
        ...
        $this->market_symbol    = $this->request->getGet('market', FILTER_SANITIZE_STRING);
        ...
        if (!$this->market_symbol) {
           $query_pair = $this->db->table('dbt_coinpair')->select('*')->where('status', 1)->orderBy('order','asc')->get()->getRow();
        ...
        $data['coin_pairs']     = $this->common_model->findAll('dbt_coinpair', array('status' => 1),  'order','asc');
        ...
        // tab data
        $data['open_trade']         = $this->common_model->findAll('dbt_biding', array('user_id' => $this->session->get('user_id'), 'status' => 2), 'open_order', 'desc', ['*', 'UNIX_TIMESTAMP(`open_order`) as unix_time']);
        $data['complete_trade']     = $this->common_model->findAll('dbt_biding', array('user_id' => $this->session->get('user_id'), 'status' => 1), 'open_order', 'desc', ['*', 'UNIX_TIMESTAMP(`open_order`) as unix_time']);
        $data['user_trade_history'] = $this->web_model->userTradeHistory($this->session->get('user_id'));

        $coin_pairs_ordered = $this->db->table('dbt_coinpair')->select('*')
            ->where('status', 1)->orderBy('order','asc')
            ->limit(30)->get()->getResult();
        $data['coin_pairs_ordered'] = $coin_pairs_ordered;
    ...
    public function withdraw($deposit_coin = null)
    {
        ...
        $this->validation->setRule('blockchain_address', 'Blockchain Address', 'required|trim');
        $this->validation->setRule('network', 'Network', 'required|trim');
        ...
        if($this->request->getMethod() == 'post'){
            ...
            if ($this->validation->withRequest($this->request)->run()){
                ...
                $fee_type = $fees_val->feetype;

                // check if balance with withdraw token and balance with fee token exists
                $balance  = $this->common_model->findById('dbt_balance', array('user_id' => $this->session->get('user_id'), 'currency_symbol' => $crypto_coin));
                $balance_fee  = $this->common_model->findById('dbt_balance', array('user_id' => $this->session->get('user_id'), 'currency_symbol' => $fee_type));
                //Fees in Percent
                $fees = calc_fees($amount, @$fees_val->fees, $fees_val->feetype);

                if ($fee_type === $crypto_coin) {
                    ...
                    $where_fee = "WEEK(`request_date`) = WEEK(CURDATE()) AND YEAR(`request_date`) = YEAR(CURDATE()) AND MONTH(`request_date`) = MONTH(CURDATE()) AND currency_symbol = '".$fee_type."' AND status !=0";

                    $balance7days_fee = $this->db->table('dbt_withdraw')->select('sum(amount)')->where($where)->where('user_id', $userinfo->user_id)->get()->GetRow();
                    ...
                    if ($userinfo->verified == 1){
                        ...
                        $trnSetup = $this->common_model->findById('dbt_transaction_setup', array('trntype' => 'WITHDRAW', 'acctype' => 'VERIFIED', 'currency_symbol' => $crypto_coin, 'status' => 1));
                        if ($trnSetup) {
                            if (@$trnSetup->upper <= (@$balance7days->amount+$amount)) {
                                $this->session->setFlashdata('exception', display('your_weekly_limit_exceeded'));
                        ...
                        $trnSetup_fee = $this->common_model->findById('dbt_transaction_setup', array('trntype' => 'WITHDRAW', 'acctype' => 'VERIFIED', 'currency_symbol' => $fee_type, 'status' => 1));
                        if ($trnSetup_fee) {
                            if (@$trnSetup_fee->upper <= (@$balance7days_fee->amount+@$fees)) {
                                $this->session->setFlashdata('exception', display('your_weekly_limit_exceeded'));
                        ...
                        $trnSetup_fee = $this->common_model->findById('dbt_transaction_setup', array('trntype' => 'WITHDRAW', 'acctype' => 'UNVERIFIED', 'currency_symbol' => $fee_type, 'status' => 1));
                    ...
                    $pending_withdraw_sum = $this->db->table('dbt_withdraw')->select('SUM(amount) as amount',FALSE)->where('status', 2)->where('user_id', $userinfo->user_id)->where('currency_symbol', $crypto_coin)->get()->getRow();
                    $pending_fees_sum = $this->db->table('dbt_withdraw')->select('SUM(fees_amount) as fees_amount',FALSE)->where('status', 2)->where('user_id', $userinfo->user_id)->where('currency_symbol', $crypto_coin)->get()->getRow();
                    $pending_amount = $pending_withdraw_sum->amount + $pending_fees_sum->fees_amount;

                    $available_balance = ((float)@$pending_amount + $amount);
                    ...
                    $pending_withdraw_sum_fee = $this->db->table('dbt_withdraw')->select('SUM(amount) as amount',FALSE)->where('status', 2)->where('user_id', $userinfo->user_id)->where('currency_symbol', $fee_type)->get()->getRow();
                    $pending_fees_sum_fee = $this->db->table('dbt_withdraw')->select('SUM(fees_amount) as fees_amount',FALSE)->where('status', 2)->where('user_id', $userinfo->user_id)->where('currency_symbol', $fee_type)->get()->getRow();
                    $pending_amount_fee = $pending_withdraw_sum_fee->amount + $pending_fees_sum_fee->fees_amount;

                    $available_balance_fee = ((float)@$pending_amount_fee + $fees);

                    if(@$balance_fee->balance < @$available_balance_fee){
                        $this->session->setFlashdata('exception', display('balance_is_unavailable'));
                        return  redirect()->to(base_url('withdraw'));
                    }
                }
    ...
    public function buy()
    {
        ...
        if ($this->session->get('isLogIn') && $this->session->get('user_id')){
            ...
            if ($fees) {
                ...
                $fees_amount = calc_fees($rate * $qty, $fees->fees, $fees->feetype);
                $buyfees     = $fees->fees;
                $buyfeetype     = $fees->feetype;
            ...
            if ($sellerfees) {
                ...
                $sellfeetype = $sellerfees->feetype;
            ...
            $pending_withdraw = $this->db->table('dbt_withdraw')
                ->select('SUM(amount)+SUM(fees_amount) as amount',FALSE)->where('currency_symbol', $coin_symbol[1])->where('status', 2)->where('user_id', $user_id)->get()->getRow();
                    ...
                    if ($sell_exchange_query) {
                        foreach ($sell_exchange_query as $key => $sellexchange) {
                            ...
                            if ($last_exchange->status == 2) {
                                ...
                                $buyer_available_qty   = (($last_exchange->bid_qty_available-$sellexchange->bid_qty_available)<=0)?0
                                                            :($last_exchange->bid_qty_available-$sellexchange->bid_qty_available);

                                $buyer_amount_available = ($last_exchange->amount_available-($sellexchange->bid_qty_available*$sellexchange->bid_price)<=0)?0
                                                            :($last_exchange->amount_available-($sellexchange->bid_qty_available*$sellexchange->bid_price));
                                $seller_amount_available = ((($sellexchange->bid_qty_available-$last_exchange->bid_qty_available)<0)?0
                                                            :$sellexchange->bid_qty_available-$last_exchange->bid_qty_available)*$sellexchange->bid_price;
                               ...
                               $exchangebuydata = array(
                                ...
                                if($last_exchange->bid_price > $sellexchange->bid_price){
                                    ...
                                    $returnfees        = 0;
                                    $byerfees          = calc_fees($totalexchanceqty*$last_exchange->bid_price, $buyfees, $buyfeetype);
                                    $sellerrfees       = calc_fees($totalexchanceqty*$sellexchange->bid_price, $sellfees, $sellfeetype);
                                ...
                                if (!$buyer_balance) {
                                    ...
                                    $this->common_model->update('dbt_balance',
                                        array('balance' => $buyer_balance->balance+$buyer_complete_qty_log),
                                        array('user_id' => $last_exchange->user_id,
                                        'currency_symbol' => $coin_symbol[0]));
                                }
                                ...
                                if (!$check_seller_balance) {
                                  ...
                                  $this->common_model->update('dbt_balance',
                                    array('balance' => $check_seller_balance->balance+($buyer_complete_qty_log*$sellexchange->bid_price)),
                                    array('user_id' => $sellexchange->user_id,
                                    'currency_symbol' => $coin_symbol[1]));
                                }
                                ...
                                $this->update_ohlc($last_exchange->market_symbol);
                            }
                            //Order running
    ...
    public function sell()
    {
        ...
        if ($this->session->get('isLogIn') && $this->session->get('user_id')){
            ...
            if ($fees) {
                ...
                $fees_amount = calc_fees($qty, $fees->fees, $fees->feetype);
                $sellfees    = $fees->fees;
                $sellfeetype    = $fees->feetype;
                            ...
                            if ($last_exchange->status == 2) {
                               ...
                               $this->common_model->update('dbt_biding', $exchangebuydata, array('id' => $buyexchange->id));
                                ...
                                if($buyexchange->bid_price>$last_exchange->bid_price){
                                    ...
                                    //Fees when Adjustment
                                    $returnfees     = 0;
                                    $byerfees       = calc_fees($totalexchanceqty*$buyexchange->bid_price, $buyfees, $buyfeetype);
                                    $sellerrfees    = calc_fees($totalexchanceqty*$last_exchange->bid_price, $sellfees, $sellfeetype);
    ...
    private function update_ohlc($market_symbol) {
        return;
        ...
        foreach ($resolutions as $resolution) {
            ...
            $open_date  = date('Y-m-d h:i').':00';
            $last = $this->db->table('dbt_coinhistory')
                    ->select('*')
                    ->where("date >= DATE_SUB('$open_date', INTERVAL $minutes minute)")
					->where('market_symbol', $market_symbol)
                    ->orderBy('date')
                    ->get()->getResultArray();
            if ($last) {
                $open = $last[0]['last_price'];
                $close = $last[count($last)-1]['last_price'];
                $xiv = $this->db->table('dbt_coinhistory')
                        ->selectSum('volume_1h', 'volume')
                        ->selectMax('last_price', 'high')
                        ->selectMin('last_price', 'low')
	    				->where('market_symbol', $market_symbol)
                        ->where("date >= DATE_SUB(NOW(), INTERVAL $minutes minute)")
                        ->orderBy('date')
                        ->get()->getRowArray();
                $high = $xiv['high'];
                $low = $xiv['low'];
                $volume = $xiv['volume'];
                $exist = $this->db->table('dbt_coinhistory_ohlc')
                    ->where('date', $open_date)
					->where('market_symbol', $market_symbol)
                    ->where('minutes', $minutes)
                    ->get()->getRow();
                if ($exist) {
                    $this->db->table('dbt_coinhistory_ohlc')
                        ->where('date', $open_date)
                        ->where('minutes', $minutes)
		    			->where('market_symbol', $market_symbol)
                        ->update([
    ...
    public function streamer_sell()
    {
        ...
        $sql = "SELECT *, SUM(`bid_qty_available`) as total_qty, SUM(`bid_qty_available`*`bid_price`) as total_price FROM dbt_biding WHERE (`status`=2 OR `status`=3) AND `market_symbol`='".$market_symbol."' AND `bid_type`='SELL' GROUP BY `dbt_biding`.`bid_price` ORDER BY `dbt_biding`.`bid_price` ASC LIMIT 10";
        $trades = $this->db->query($sql, [])->getResult();
    ...
    public function tradehistory()
    {
        ...
        $tradehistory       = $this->db->table('dbt_biding_log')
            ->select('dbt_biding_log.*,UNIX_TIMESTAMP(success_time)*1000 `timestamp`, IF(success_time=CURDATE(),TIME(success_time),CONCAT(DAY(success_time), ",", TIME(success_time))) as successtime')
            ->where('market_symbol', $market_symbol)->where('status', 1)
            ->limit(0,25)->orderBy('success_time', 'desc')->get()->getResult();
    ...
    ******************************/
    ...
    public function fees_load($amount=null,$method=null,$level,$coin=null)
    {
        ...
        $result = $this->db->table('dbt_fees')
            ...
            ->getRow();
        if ($feetype) {
            return calc_fees($amount, $result->fees, $result->feetype);
        }
        return 0;
    }
    ...
    */
    ...
    public function coingecko($value='')
    {
        # code...
        $coin_return = coinMarketCap();
        print_r($coin_return);
        exit;
    }

    public function coinmarketcap($value='')
    {
        # code...
        $coin = new CoinMarketCap();
        print_r($coin->cryptocurrency_ohlcv_latest());
        exit;
    }

    public function open_order_json()
    {
        $user_id = $this->session->get('user_id');
        $sql = "SELECT *, UNIX_TIMESTAMP(`open_order`)*1000 as unix_time FROM dbt_biding WHERE (`user_id`='$user_id') AND `status`=2 ORDER BY `unix_time` DESC";
        $trades = $this->db->query($sql, [])->getResult();

        echo json_encode(array('trades' => $trades));
    }

    public function complete_trade_json()
    {
        $user_id = $this->session->get('user_id');
        $sql = "SELECT *, UNIX_TIMESTAMP(`open_order`)*1000 as unix_time FROM dbt_biding WHERE (`user_id`='$user_id') AND `status`=1 ORDER BY `unix_time` DESC";
        $trades = $this->db->query($sql, [])->getResult();

        echo json_encode(array('trades' => $trades));
    }

    public function user_trade_history_json()
    {
        $user_id = $this->session->get('user_id');
