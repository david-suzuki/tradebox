...
class Web_model
{
  ...
  public function userTradeHistory($user_id = null)
  {
    return $this->db->table('dbt_biding bidmaster')
      ->select('bidmaster.*, UNIX_TIMESTAMP(bidmaster.open_order)*1000 as unix_time, biddetail.bid_type as bid_type1, biddetail.bid_price as bid_price1, biddetail.market_symbol as market_symbol1, biddetail.complete_amount as complete_amount1, biddetail.success_time as success_time1, biddetail.complete_qty, biddetail.complete_amount, biddetail.success_time, UNIX_TIMESTAMP(biddetail.success_time)*1000 as unix_success')
      ->join('dbt_biding_log biddetail', 'biddetail.bid_id = bidmaster.id', 'left')
      ->where('bidmaster.user_id', $user_id)
      ->orderBy('bidmaster.id', 'desc')
      ->get()
      ->getResult();
  }
