...
$(function ($) {
  ...
  if (path == "") {
    ...
    $("#subscribeForm").on("submit", function (event) {
      ...
      if (email == "") {
        allert_warning(
          "warning",
          obj["please_enter_valid_email"][BDTASK.language()]
        );
        return false;
      }
      ...
      $.ajax({
        ...
        error: function (xhr, status, error) {
          if (xhr.status === 500) {
            allert_warning(
              "warning",
              obj["already_subscribe"][BDTASK.language()]
            );
          }
        },
      });
  ...
  if (path == "profile-verify") {
    $("#verify_type").on("change", function (event) {
      ...
      if (verify_type == "passport") {
        ...
        $("#verify_field").html(
          "<div class='form-group row'><label for='document1' class='col-md-4 col-form-label'>Driving License </label><div class='col-md-8'><input name='document1' type='file' class='form-control' id='document1' required></div></div>"
        );
      } else if (verify_type == "nid") {
        $("#verify_field").html(
          "<div class='form-group row'><label for='document1' class='col-md-4 col-form-label'>NID With selfie </label><div class='col-md-8'><input name='document1' type='file' class='form-control' id='document1' required></div></div>"
        );
      } else {
        $("#verify_field").html();
      }
  ...
  $("body").on("submit", "#contactForm", function (event) {
    ...
    $.ajax({
      ...
      success: function (d) {
        allert_warning(
          "success",
          obj["message_send_successfuly"][BDTASK.language()]
        );
        location.reload();
      },
      error: function () {
  ...
  if (path == "deposit") {
    $("#deposit_type").on("change", function (event) {
      ...
      if (deposit_type == "coin") {
        $.getJSON(BDTASK.getSiteAction("dafult-data"), function (data) {
          var htmlCoin =
            "<option>" + obj["select_option"][BDTASK.language()] + "</option>";
        ...
        $("#crypto_coin").on("change", function (event) {
          ...
          if (crypto_coin == "BTC") {
            $.getJSON(BDTASK.getSiteAction("dafult-data"), function (data) {
              var htmlGateway =
                "<option>" +
                obj["deposit_method"][BDTASK.language()] +
                "</option>";
              $.each(data.payment_gateway, function (index, v) {
                if (
                  v.identity == "bitcoin" ||
                  v.identity == "coinpayment" ||
                  v.identity == "payeer" ||
                  v.identity == "token"
                ) {
                  htmlGateway +=
        ...
        $("#crypto_coin").html(
          "<option value=''>" +
            obj["select_option"][BDTASK.language()] +
            "</option><option value='USD' >US Dollar</option>"
        );
        $("#crypto_coin").on("change", function (event) {
          var crypto_coin = $("#crypto_coin").val() || 0;
          if (crypto_coin == "USD") {
    ...
    $("#payment_method").on("change", function (event) {
      ...
      if (payment_method == "phone") {
        $(".payment_info").html(
          "<div class='form-group'><label for='send_money'>Send Money</label><h2><a href=tel:" +
            gateway.public_key +
            ">" +
            gateway.public_key +
            "</a></h2></div><div class='form-group'><label for='om_name'>" +
            obj["om_name"][BDTASK.language()] +
            "</label><input name='om_name' class='form-control om_name' type='text' id='om_name' autocomplete='off'></div><div class='form-group'><label for='om_mobile'>" +
            obj["om_mobile_no"][BDTASK.language()] +
            "</label><input name='om_mobile' class='form-control om_mobile' type='text' id='om_mobile' autocomplete='off'></div><div class='form-group'><label for='transaction_no'>" +
            obj["transaction_no"][BDTASK.language()] +
            "</label><input name='transaction_no' class='form-control transaction_no' type='text' id='transaction_no' autocomplete='off'></div><div class='form-group'><label for='idcard_no'>" +
            obj["idcard_no"][BDTASK.language()] +
            "</label><input name='idcard_no' class='form-control idcard_no' type='text' id='idcard_no' autocomplete='off'></div>"
        );
      } else if (payment_method == "bank") {
        $(".payment_info").html(
          "<div class='form-group inliner'><label for='send_money'>Account Name : </label><h4>" +
            gateway_bank.acc_name +
            "</h4></div><div class='form-group inliner'><label for='send_money'>Account No :</label><h4>" +
            gateway_bank.acc_no +
            "</h4></div><div class='form-group inliner'><label for='send_money'>Branch Name : </label><h4>" +
            gateway_bank.branch_name +
            "</h4></div><div class='form-group inliner'><label for='send_money'>SWIFT Code : </label><h4>" +
            gateway_bank.swift_code +
            "</h4></div><div class='form-group inliner'><label for='send_money'>ABN No : </label><h4>" +
            gateway_bank.abn_no +
            "</h4></div><div class='form-group inliner'><label for='send_money'>Country : </label><h4>" +
            gateway_bank.country +
            "</h4></div><div class='form-group inliner'><label for='send_money'>Bank Name : </label><h4>" +
            gateway_bank.bank_name +
            "</h4></div><div class='form-group inliner'><label for='document'>Document : </label><h4><input name='document' class='form-control document' type='file' id='document' autocomplete='off'></h4></div>"
        );
      } else if (payment_method == "token") {
        $(".payment_info").html(
          "<div class='form-group'><label for='comment' class=''>Your Wallet</label><textarea class='form-control' name='comment' id='comment' rows='1'></textarea></div>"
        );
      } else {
        $(".payment_info").html(
          "<div class='form-group'><label for='comment' class=''>" +
            obj["comments"][BDTASK.language()] +
            "</label><textarea class='form-control' name='comment' id='comment' rows='3'></textarea></div>"
        );
      }
    });
  ...
  if (path == "withdraw") {
    ...
    //           var htmlGateway =
    //             "<option>" +
    //             obj["withdraw_method"][BDTASK.language()] +
    //             "</option>";
    //           $.each(data.payment_gateway, function (index, v) {
    //             if (
    //               v.identity == "bitcoin" ||
  ...
  function Fee() {
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data.feetype) {
          $('[name="fees"]').val(data.fees);
          $("#fee").text("Fees is " + data.fees + " " + data.feetype);
          if (amount === "" || amount === 0) {
            $("#fee").text("Fees is " + 0);
          }
        ...
        if (data) {
          if (method == "bank") {
            ...
            $("#walletidis").html(
              "<small>Account Name: " +
                bank.acc_name +
                "</small><br><small>Account No: " +
                bank.acc_no +
                "</small><br><small>Branch Name: " +
                bank.branch_name +
                "</small><br><small>SWIFT Code: " +
                bank.swift_code +
                "</small><br><small>ABN No: " +
                bank.abn_no +
                "</small><br><small>Country: " +
                bank.country +
                "</small><br><small>Bank Name: " +
                bank.bank_name +
                "</small><br>"
            );
          } else {
          ...
          if (method == "coinpayment" || method == "token") {
            $("button[type=submit]").prop("disabled", false);
            $("#coinwallet").html(
              "<div class='form-group'><label for='amount'>Your Address</label><input class='form-control' name='wallet_address' type='text' id='wallet_address'></div>"
            );
            $("#walletidis").text("");
          } else {
            $("#coinwallet").html("");
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data != "") {
        swal({
          title: "Good job!",
          text: "Your Withdraw Request Send Successfully",
          type: "success",
          showConfirmButton: false,
          timer: 1500,
        });
      ...
      success: function (response) {
        if (response != "") {
          ...
          window.location.replace(
            BDTASK.getSiteAction("transfer_details/" + response)
          );
        } else {
          swal({
            title: "Wops!",
...
$(document).ready(function () {
  ...
  $.getJSON(BDTASK.getSiteAction("dafult-data"), function (data) {
    ...
    $.each(data.coin_list, function (index, element) {
      if (element.symbol != "USD") {
        htmlCoin += `<option value="${element.symbol}">${
          element.symbol === "USDC" ? element.coin_name : element.full_name
        }</option>`;
      }
    });
    $("#crypto_coin").html(htmlCoin);
  ...
  $("#crypto_coin").on("change", function (event) {
    ...
    if (crypto_coin == "BTC") {
      $.getJSON(BDTASK.getSiteAction("dafult-data"), function (data) {
        var htmlGateway =
          "<option>" + obj["withdraw_method"][BDTASK.language()] + "</option>";
        $.each(data.payment_gateway, function (index, v) {
          if (
            v.identity == "bitcoin" ||
            v.identity == "coinpayment" ||
            ...
            htmlGateway +=
              "<option value=" + v.identity + " >" + v.agent + "</option>";
          }
        });
        $("#payment_method").html(htmlGateway);
      });
