(function ($) {
  ...
  $.getJSON(BDTASK.getSiteAction("get-stream"), function (datapack) {
    ...
    var dataUnpack = function (data) {
      ...
      // console.log(currentPrice[pair]["CHANGE24HOUR"]);
    ...
    var displayData = function (current, from, tsym, fsym) {
      ...
      for (var key in current) {
        if (key == "CHANGE24HOURPCT") {
          ...
          // console.log("#" + key + "_" + from, current[key]);
        } else if (key == "LASTVOLUMETO" || key == "VOLUME24HOURTO") {
          $("#" + key + "_" + from).text(
            CCC.convertValueToDisplay(tsym, current[key])
          );
        } else if (
          key == "LASTVOLUME" ||
          key == "VOLUME24HOUR" ||
          ...
          $("#" + key + "_" + from).text(
            CCC.convertValueToDisplay(fsym, current[key])
          );
        } else {
          $("#" + key + "_" + from).text(current[key]);
        }
