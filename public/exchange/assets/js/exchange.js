...
$(function ($) {
  ...
  doneMessageChat();
  doneUpdateSell();
  doneTradeHistoryUpdate();
  doneOpenOrderUpdate();
  doneUserCompleteTradeUpdate();
  doneUserTradeHistoryUpdate();
  ...
  function doneTradeHistoryUpdate() {
    setTimeout(function () {
      tradehistoryupdates();
      doneTradeHistoryUpdate();
    }, 10000);
  ...
  function doneUserCompleteTradeUpdate() {
    setTimeout(function () {
      userCompleteTradeUpdate();
      doneUserCompleteTradeUpdate();
    }, 10000);
  }

  function doneUserTradeHistoryUpdate() {
    setTimeout(function () {
      userTradeHistoryUpdate();
      doneUserTradeHistoryUpdate();
    }, 10000);
  }
  ...
  function tradehistoryupdates() {
    $.getJSON(
      BDTASK.getSiteAction("tradehistory?market=" + market),
      function (data) {
        $("#tradeHistory").empty();
        var lastprice;
        if (data.secondLast) {
        ...
        $.each(data.tradehistory, function (index, element) {
          ...
          var now = new Date();
          var showday = now.toDateString() != date.toDateString();
          var datestring = new Intl.DateTimeFormat("en-AU", {
            day: showday ? "numeric" : undefined,
            hour: "numeric",
            minute: "numeric",
            second: "numeric",
            hour12: false,
            timeZone: tv.activeChart().getTimezone(),
          }).format(date);
          $("#tradeHistory").append(
          ...
          // console.log(tv._iFrame.contentDocument.timeline.currentTime)
          //Max Row Show From Stemar
          var maxTableRow = 18;
  ...
  //fetch the trade data for last price
  let rate = 0;
      ...
      function (data) {
        if (data.coinhistory) {
          if (
            ...
            var last_price = data.coinhistory.last_price || 0;
            $("#buypricing").val(parseFloat(last_price).toFixed(6).toString());
            $("#sellpricing").val(parseFloat(last_price).toFixed(6).toString());
            rate = parseFloat(last_price).toFixed(6);
          }
        }
      }
    );
  ...
  $("#buyform").on("submit", function (event) {
    ...
    if (amount <= 0 || price <= 0) {
      ...
      $("#buyButton").html("Buy " + market_details.currency_symbol);
    ...
    $.ajax({
      ...
      success: function (data) {
        if (data == 0) {
          ...
          if (isNaN(trade.balance) || trade.balance == null) trade.balance = 0;
          if (isNaN(trade.balance_up_to) || trade.balance_up_to == null)
            trade.balance_up_to = 0;
...
function openOrderUpdate() {
  $.getJSON(BDTASK.getSiteAction("open-order-json"), function (data) {
    $("#openOrder").empty();

    $.each(data.trades, function (index, element) {
      date = new Date(Number(element.unix_time));
      var datestring = new Intl.DateTimeFormat("en-AU", {
        year: "numeric",
        month: "numeric",
        day: "numeric",
      ...
      $("#openOrder").append(
        "<tr><td>" +
          ...
          parseFloat(element.bid_price).toFixed(8) +
          "</td><td>" +
          parseFloat(element.bid_qty).toFixed(8) +
          "</td><td>" +
          parseFloat(element.bid_qty_available).toFixed(8) +
          "</td><td>" +
          parseFloat(element.total_amount).toFixed(8) +
          "</td><td>" +
          parseFloat(element.amount_available).toFixed(8) +
          "</td><td>" +
          element.market_symbol +
          "</td><td>" +
          datestring +
          "</td><td width='30'><div class='progress'><div title='Running' class='progress-bar progress-bar-striped width100percent'>Running</div></div>" +
          '</td><td><a href="/order-cancel/' +
          element.id +
          '" class="bg-danger text-white text-center" data-toggle="tooltip" data-placement="left" title="Cancel">Cancel</a>' +
          "</td></tr>"
      );
    });
  });
}
function userCompleteTradeUpdate() {
  $.getJSON(BDTASK.getSiteAction("complete-trade-json"), function (data) {
    $("#usercompleteTrade").empty();
    ...
    $.each(data.trades, function (index, element) {
      ...
      $("#usercompleteTrade").append(
        "<tr><td>" +
          ...
          "</td><td>" +
          datestring +
          "</td><td class='text-left'><i title='Success' class='feather-check-circle text-success'></i>Success</td></tr>"
      );
    });
  });
}
function userTradeHistoryUpdate() {
  $.getJSON(BDTASK.getSiteAction("user-trade-history-json"), function (data) {
    $("#userTradeHistory").empty();
    ...
    $.each(data.trades, function (index, element) {
      ...
      success = new Date(
        Number(element.unix_success ? element.unix_success : element.unix_time)
      );
      var successdatestring = new Intl.DateTimeFormat("en-AU", {
        year: "numeric",
        month: "numeric",
        day: "numeric",
        hour: "numeric",
      ...
      $("#userTradeHistory").append(
        "<tr><td>" +
          ...
          datestring +
          "</td><td>" +
          (element.complete_qty == null
            ? "0.0"
            : parseFloat(element.complete_qty).toFixed(8)) +
          "</td><td>" +
          (element.complete_amount == null
            ? "0.0"
            : parseFloat(element.complete_amount).toFixed(8)) +
          "</td><td>" +
          successdatestring +
          "</td><td>" +
          (element.status == 0
            ? "<i title='Canceled' class='feather-x-circle text-danger'></i>Canceled"
            : element.status == 1
            ? "<i title='Complete' class='feather-check-circle text-success'></i>Complete"
